...
@Component({
  ...
  template: `
    <div class="p-6">
      <h1 class="text-surface-700 dark:text-surface-200 mb-6 text-2xl font-semibold" id="casual-heading">
        Casual Games
      </h1>
      <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        @for (game of casualGames; track game.routerLink) {
          <p-card class="border border-surface-200 dark:border-surface-700 shadow-sm">
            <ng-template pTemplate="header">
              <h2 class="text-surface-700 dark:text-surface-200 px-4 py-3 text-lg font-medium">
                {{ game.title }}
              </h2>
            </ng-template>
            <p class="text-surface-600 dark:text-surface-400 px-4 pb-2 text-sm">{{ game.description }}</p>
